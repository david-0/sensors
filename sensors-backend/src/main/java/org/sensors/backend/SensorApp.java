...
public class SensorApp {
	...
	public void init(StateStore stateStore) {
		try {
			...
			sensorController = new SensorController((id, value) -> stateStore.update(id, value));
			createMcp9808Sensors(bus).stream().forEach(sensorController::addIntervalBasedSource);
			createIna219Sensors(bus).stream().forEach(sensorController::addIntervalBasedSource);
			createOneWireSensors(master).stream().forEach(sensorController::addIntervalBasedSource);

			sensorController.addIntervalBasedSource(new SensorMe2O2(bus, 0x04, "o2", "O2 Sensor", 84.0).init());
			sensorController
					.addIntervalBasedSource(new SensorSHT31d(bus, 0x44, "sht31d", "temp / humidity Sensor").init());
			sensorController.addIntervalBasedSource(
